...
export class Movie {
  ...
  borshInstructionSchema = borsh.struct([
    borsh.u8("variant"), // represents which ixn should be executed
    borsh.str("title"),
    borsh.u8("rating"), // rating out of 5
    borsh.str("description"),
  ]);

  serialize(): Buffer {
    const buffer = Buffer.alloc(1000); // Create oversized buffer

    // Use schema to encode data into buffer
    this.borshInstructionSchema.encode({ ...this, variant: 0 }, buffer);

    // Remove extra unused space in the buffer and return
